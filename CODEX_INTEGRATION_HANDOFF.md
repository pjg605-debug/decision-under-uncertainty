...
`import_pending_case()` itself is still hard-capped at `RESEARCH_DONE` (it raises an exception if asked to create a case at any later status) — case creation alone never reaches the public site. **What changed later the same day**: see "Automated approval/publish" immediately below — the review step this section originally said Codex still owned exclusively has since been removed for narratives submitted through Claude's own pipeline.

Practical effect for Codex: `get_cases_for_research`/`get_cases_for_narrative` may now surface cases Claude created, not only ones Codex created. The 16-case research pilot in `data/cases/<case-id>/` (`CLAUDE_PILOT_TO_DB_MAPPING.md`) is the first backlog Claude is drawing from, one case at a time, canary-tested before any batch — check that mapping doc (or `get-narrative-queue-status`) before doing manual case-creation work yourself, to avoid a race on the same case.

## Automated approval/publish: Codex review is no longer in this loop (product owner, 2026-09-03)

Later the same day as the case-creation change above, the product owner explicitly asked to remove the remaining human/Codex review checkpoint too, was told plainly what that gives up (an independent check before content reaches the public site), and confirmed the fully-automated option anyway.

**What changed:** `scripts/submit-pending-narratives.mjs`'s `submitOne` now calls `editorial-desk.mjs`'s new `autoApproveAndPublish()` immediately after a narrative insert is verified, before archiving the draft file. It replicates every safety property `app/api/review/route.ts`'s human-triggered approve endpoint had:

- Opens a `reviews` row with `verdict = 'APPROVE'`, but `reviewer_agent` is the automation (`claude`), not a human or `codex` -- greppable/auditable as non-human approval after the fact.
- Sets `is_current = true` **scoped by `case_id` AND `language`**, clearing only the same language's previous current flag -- never the sibling language's, same bug class already fixed once in the human-triggered path.
- Walks the case through whichever of `CODEX_REVIEW → APPROVED → PROTOTYPE_READY → PUBLISHED` it hasn't already reached (idempotent: adding a second language to an already-`PUBLISHED` case is a no-op transition-wise).

**What this means for Codex:** `get_cases_for_codex_review` will generally stay empty for cases that came through Claude's file-based pipeline -- they no longer pause at `CODEX_REVIEW` waiting for a verdict. The review dashboard (`app/api/review/route.ts`, `app/review/page.tsx`) and `submit-review` CLI command still exist and still work exactly as before for anything routed through them by hand; nothing about that path was removed, it's just no longer the only path to `PUBLISHED`. If Codex still wants to spot-check Claude-authored content, do it after publish (it's live, same as any other published case) rather than expecting a pending queue to review first.

This does not touch case creation's own boundary (still capped at `RESEARCH_DONE`, see above) or any of the "always apply" editorial rules in `CLAUDE.md` (T0 discipline, decision quality vs. outcome quality, genuine dilemma, evidence discipline) -- those still constrain what Claude writes, there is simply no second reviewer confirming it anymore before it goes live.
