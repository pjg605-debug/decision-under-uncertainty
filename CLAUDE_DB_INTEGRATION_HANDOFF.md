...
- The `decision_options`/`case_information`/`evidence` structured tables are **not yet bilingual** -- this migration deliberately did not extend the language dimension to them (see `CODEX_INTEGRATION_HANDOFF.md`'s categorization of which fields are language-neutral vs. player-facing text needing a KO/EN pair). Only `narratives` prose is currently split by language.
