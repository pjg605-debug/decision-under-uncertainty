...
6. **As of 2026-09-03 (product owner decision), narratives submitted through `scripts/submit-pending-narratives.mjs` are approved and published automatically** (`editorial-desk.mjs`'s `autoApproveAndPublish`, called right after a successful insert) -- no human or Codex review step. It replicates every safety property the old human-triggered flow had: it opens a `reviews` row (verdict `APPROVE`, attributed to the automation, not a human), sets `is_current = true` **for its language only** (clearing the previous current flag for that same language only, never the sibling language), and walks the case forward through whatever of `CODEX_REVIEW`/`APPROVED`/`PROTOTYPE_READY`/`PUBLISHED` it hasn't already reached. This is a deliberate removal of the review checkpoint -- see CODEX_INTEGRATION_HANDOFF.md's "Automated approval/publish" section for the full rationale and what Codex should know.
