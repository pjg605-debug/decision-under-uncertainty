...
-- Fixes a real bug found on first use of import_pending_case(): its
-- `returns table (case_id uuid, case_key text, status text)` OUT
-- parameters shadow the identically-named columns on decision_cases, so
-- `where case_key = v_case_key` inside the duplicate check is ambiguous
-- between the table column and the OUT parameter (Postgres 42702). This
