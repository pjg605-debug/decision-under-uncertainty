...
export default async function ArticlePage({ params }: Props) {
  ...
  const article = await fetchPublishedArticle(slug);
  if (!article) {
    return <main className="min-h-screen bg-background"><ArticleHeader backToArchive /><div className="mx-auto max-w-3xl px-4 py-24"><h1 className="text-3xl font-semibold">아티클을 찾을 수 없습니다.</h1></div></main>;
  }
  const published = new Intl.DateTimeFormat('ko-KR', { dateStyle: 'long', timeZone: 'Asia/Seoul' }).format(new Date(article.published_at));
  return (
      ...
      <article>
        <header className="border-b bg-card/50">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:py-20">
            <p className="font-mono text-xs uppercase tracking-[.2em] text-primary">{article.hero_kicker}</p>
        ...
        <div className="mx-auto max-w-3xl px-4 py-12 sm:py-16">
          <div className="space-y-6 text-[1.05rem] leading-8">
            {article.content_blocks.map((block, index) => {
              if (block.kind === 'heading') return <h2 key={index} className="pt-7 text-3xl font-semibold leading-tight tracking-[-.03em]">{block.text}</h2>;
              if (block.kind === 'paragraph') return <p key={index}>{block.text}</p>;
              if (block.kind === 'quote') return <blockquote key={index} className="my-10 border-l-2 border-primary pl-6 text-xl font-medium leading-9 text-primary">{block.text}</blockquote>;
              if (block.kind === 'callout') return <aside key={index} className="my-10 rounded-2xl border border-primary/25 bg-primary/5 p-6"><p className="eyebrow text-primary">{block.title}</p><p className="mt-3 leading-8">{block.text}</p></aside>;
              return <ul key={index} className="grid gap-3 pl-1">{block.items.map((item) => <li key={item} className="flex gap-3"><span className="mt-3 h-1.5 w-1.5 shrink-0 rounded-full bg-primary" /><span>{item}</span></li>)}</ul>;
            })}
          </div>
          <section className="mt-16 rounded-[1.7rem] border bg-card p-6 sm:p-8">
            <p className="eyebrow flex items-center gap-2 text-primary"><FlaskConical size={14} /> 사고 실험</p>
            <h2 className="mt-3 text-2xl font-semibold">{article.thought_experiment.title}</h2>
            <p className="mt-4 leading-7">{article.thought_experiment.prompt}</p>
            <ol className="mt-5 grid gap-2">{article.thought_experiment.choices.map((choice, i) => <li key={choice} className="rounded-xl border bg-background p-4 text-sm"><b className="mr-2 text-primary">{String.fromCharCode(65 + i)}.</b>{choice}</li>)}</ol>
